...
P3 已通过验收：房间地面/顶面、标高变化、柱、梁、楼梯、UV、GLB 坐标元数据与二进制结构校验，以及 20 个固定几何样例、场景哈希和编译场景顶视对齐均已完成。复杂墙网格采用普通转角斜接、T/X 交点的贯通墙/支墙贴面，以及共线重叠墙段的切分去重；验收会阻断未解析交点和墙体体积重叠。P4 已正式开放；本文对 P5-P10 的描述仍只是目标设计，不构成阶段开放。
