...
- 当前开发阶段是 **P10：生产化、跨平台与发布（IN_PROGRESS）**。
- P2-P9 已完成；P10 正在把既有能力纳入可恢复运行时、跨平台、安全、性能和发布资格门禁。
...
| **P10** | **`IN_PROGRESS`** | 生产运行时、跨平台资格、安全、性能、文档和发布门禁正在实现 |
...
P3-P7 已分别完成确定性毛坯几何、PBR/硬装、精装资产、Viewer/WebXR，以及 Blender/全景交付。P8 已完成受限自然语言修订、真实补丁、增量执行和哈希链版本审计。P9 已完成 DWG/PDF/IFC、视觉媒体、点云/深度、已有 3D 和产品目录的统一证据与失败门禁。无 Blender、跨平台宿主或物理头显的当前环境不会冒充目标设备测试；这些真实执行资格、生产运行时、安全审计和正式发布结论由已开放的 P10 统一管理。
