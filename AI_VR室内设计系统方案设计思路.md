...
- 当前开发阶段是 **P7：Blender、多视角与 360°（IN_PROGRESS）**。
- P2-P6 已完成；P8 及以后能力即使已有演示代码，也只能视为冻结的预研原型。
...
| **P7 Blender 与 360°** | **`IN_PROGRESS`** | 后台组装、多机位和全景交付正在实现 |
| P8-P10 | `BLOCKED` | 修订、输入扩展和生产化 |
...
P3 已通过验收：房间地面/顶面、标高变化、柱、梁、楼梯、UV、GLB 坐标元数据与二进制结构校验，以及 20 个固定几何样例、场景哈希和编译场景顶视对齐均已完成。复杂墙网格采用普通转角斜接、T/X 交点的贯通墙/支墙贴面，以及共线重叠墙段的切分去重；验收会阻断未解析交点和墙体体积重叠。P4 已正式开放：PBR 材质槽/颜色空间/铺贴尺度契约、稳定 ID 绑定的硬装构件、可追踪的材质替换、相对本地纹理和数据 URI 的 GLB 嵌入、可追踪的缺失纹理回退，以及 `KHR_lights_punctual` 的基础光照和三档质量配置均已实现。P4 的 20 个固定样本已验证 Web/Blender 的 glTF 导入契约（不调用 Blender，不越过 P7）；性能证据和人工验收仍未完成。本文对 P5-P10 的描述仍只是目标设计，不构成阶段开放。
