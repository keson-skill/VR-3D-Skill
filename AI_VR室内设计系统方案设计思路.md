...
- 当前开发阶段是 **P10：生产化、跨平台与发布（ACCEPTANCE）**。
- P2-P9 已完成；P10 的生产运行时、安全、迁移、性能、交付和发布代码已通过验收，正在补同一最终提交的三平台、真实设备/Blender 和匿名真实项目资格。
...
P9 已完成全部目标输入的正式路由和质量边界：DWG 经过批准的本地 DXF 转换；PDF 按页区分矢量、扫描与混合证据；照片、视频和全景使用尺度/相机门禁；IFC 保留楼层、构件、属性与单位；点云、深度和已有 3D 统一轴、尺度、资源与来源证据。简单确定性路线可生成待批准 Spatial 草案，复杂输入会明确要求本地几何引擎、配准或人工映射，绝不把“已识别格式”宣传成“已可靠重建”。
...
P10 不引入第二套空间或设计逻辑，而是在既有 P2-P9 管线外围增加生产控制面：

```mermaid
flowchart TD
    A["白名单生产作业"] --> B["幂等、锁、重试与检查点"]
    B --> C["批准、场景、XR 与 Blender 处理器"]
    C --> D["交付清单与许可证门禁"]
    D --> E["三平台和真实目标资格"]
    E --> F["SBOM、发布包、清单与标签验证"]
```

生产运行时保存原子状态、阶段检查点和哈希链事件；同一幂等键不能绑定不同定义。排他锁带随机所有权令牌，支持心跳与已验证陈旧恢复，旧工作者不能删除新锁。日志只允许稳定 ID、错误码、耗时、请求 ID、模型/工具版本和哈希，密钥、客户联系方式与本地路径必须删减。作业定义只允许准备输入、验证空间、等待独立批准、编译场景、验证 XR 和 Blender 渲染六类处理器，不提供任意 shell；所有输出必须位于显式生产工作区并拒绝符号链接穿越。

生产资格分为四层，彼此不能替代：

1. 30 个固定正常/边界/失败样例与全仓回归证明代码合同；
2. 静态场景预算和当前宿主核心基准证明确定性与资源上限；
3. Linux、macOS、Windows CI 证明同一提交可安装和执行；
4. 物理桌面、移动、XR、实际 Blender/GPU 和匿名真实项目证明目标交付。

正式交付清单必须绑定来源、Spatial JSON、验证、人工批准、信任根、资产清单和每个交付物哈希；精装资产许可证未解决时阻断。发布包必须包含覆盖全部锁定组件和依赖图的 CycloneDX SBOM 与哈希清单。资格包内嵌可复算的平台、目标捕获及真实项目接受记录，但大体积原始证据外部留存；资格包不提交回仓库，而以受保护 secret 传给同一提交的标签工作流，从而避免提交/证据循环。只有资格包、干净提交、版本和正式标签完全一致时才能标记 `release_ready`，否则只能生成带明确非声明的候选包。哈希证明完整性和提交绑定，不单独证明物理采集真实发生。
...
| **P3 确定性毛坯建模与 GLB** | **`COMPLETED`** | 确定性毛坯几何、GLB 契约和 20 个固定回归样本已通过验收 |
| **P4 硬装与材质管线** | **`COMPLETED`** | PBR、硬装、材质替换、纹理/灯光与 20 样本审计通过 |
| **P5 精装设计与资产** | **`COMPLETED`** | 两方案、布局净空、目录资产和代理闭环通过 20 样本验收 |
| **P6 Viewer 与 WebXR** | **`COMPLETED`** | 导航、交互、生命周期和可访问性通过 20 样本验收 |
| **P7 Blender 与 360°** | **`COMPLETED`** | 后台组装、多机位、全景/播放器、可选漫游、哈希和恢复合同通过 20 样本验收 |
| **P8 修订与版本** | **`COMPLETED`** | 受限规划、补丁应用、增量依赖、重批准、撤销/回滚和哈希审计通过 20 样本验收 |
| **P9 全输入扩展** | **`COMPLETED`** | 11 类 CAD/PDF/BIM、视觉、扫描、点云、深度、已有 3D 与目录输入通过 33 个固定样本和真实工具集成验收 |
| **P10** | **`ACCEPTANCE`** | 生产运行时、安全、迁移、性能、交付、CI 和发布门禁已通过代码验收；真实资格待执行 |
...
P3-P7 已分别完成确定性毛坯几何、PBR/硬装、精装资产、Viewer/WebXR，以及 Blender/全景交付。P8 已完成受限自然语言修订、真实补丁、增量执行和哈希链版本审计。P9 已完成 DWG/PDF/IFC、视觉媒体、点云/深度、已有 3D 和产品目录的统一证据与失败门禁。P10 已完成可恢复运行时、工作区隔离、输入/供应商/进程/Viewer 加固、安全迁移、静态预算、完整资格记录、三平台 CI 配置、交付清单、全锁定依赖 SBOM 和候选/生产发布门禁；99 项全仓测试和 30 项 P10 固定样例已通过。无跨平台运行产物、物理移动/XR、真实 Blender/GPU 或匿名真实项目的当前环境不会冒充这些资格；因此 P10 保持 `ACCEPTANCE`，正式发布结论仍被阻断。
...
用户上传经过尺度与隐私门禁的房间资料后，系统可导入经过许可证和尺寸校验的产品目录、生成视觉预演，并在独立批准后生成可交互 3D；观察不到的空间仍保持显式推断。
