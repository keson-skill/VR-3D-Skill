...
- 当前开发阶段是 **P8：自然语言修订、增量生成与版本（IN_PROGRESS）**。
- P2-P7 已完成；P9 及以后能力即使已有演示代码，也只能视为冻结的预研原型。
...
| **P8 修订与版本** | **`IN_PROGRESS`** | 稳定 ID 补丁、增量依赖、撤销/回滚和审计正在实现 |
| P9-P10 | `BLOCKED` | 输入扩展和生产化 |
...
P3-P6 已分别完成确定性毛坯几何、PBR/硬装管线、精装布局与真实资产解析，以及统一 Viewer/WebXR 运行时合同。P7 已完成 Blender 后台计划和执行脚本、标准与自定义机位、1080p 静帧、4096×2048 全景、独立全景播放器、可选漫游视频、`.blend`、实际版本记录、逐产物哈希和原子恢复检查点，并以 20 个固定样本和可执行编排测试通过代码验收。无 Blender 或物理头显的 CI 环境不冒充目标设备测试，真实 Blender/GPU/XR 发布资格统一留给 P10。本文对 P9-P10 的描述仍只是目标设计，不构成阶段开放。
