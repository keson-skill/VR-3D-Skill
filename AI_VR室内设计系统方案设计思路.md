...
- 当前开发阶段是 **P9：全输入扩展（IN_PROGRESS）**。
- P2-P8 已完成；P10 能力即使已有演示代码，也只能视为冻结的预研原型。
...
| **P9 全输入扩展** | **`IN_PROGRESS`** | CAD/PDF/BIM、视觉、扫描、点云、深度和已有 3D 输入适配正在实现 |
| P10 | `BLOCKED` | 生产化、跨平台资格和发布 |
...
P3-P7 已分别完成确定性毛坯几何、PBR/硬装、精装资产、Viewer/WebXR，以及 Blender/全景交付。P8 已完成受限自然语言修订规划、真实补丁应用、保留条件、依赖与增量执行、批准失效、差异、反向修订、撤销、历史回滚和哈希链审计，并以 20 个固定样本及版本篡改测试通过代码验收。无 Blender 或物理头显的 CI 环境不冒充目标设备测试，真实 Blender/GPU/XR 发布资格统一留给 P10。本文对 P10 的描述仍只是目标设计，不构成阶段开放。
