...
- 当前开发阶段是 **P2：可信空间重建与人工批准链（IN_PROGRESS）**。
...
- 结合本地图片、OCR 和 CAD 证据生成 Spatial JSON 草案；
...
| **P2 可信空间重建与批准链** | **`IN_PROGRESS`** | 确定性语义提取、校正、验证和人工批准 |
| P3-P10 | `BLOCKED` | 场景、硬装、资产、Viewer、Blender、修订、输入扩展和生产化 |

P2 当前仍缺少：

- DXF 到墙体、门窗和房间的确定性语义映射；
- 栅格图墙线、开口、轮廓和尺度锚点提取；
- 2D 叠加人工校正；
- 哈希绑定的人工批准文件；
- 不少于 5 个栅格和 5 个 DXF 固定样本；
- 源图与标准顶视图的对齐验收。
