...
- 当前开发阶段是 **P2：可信空间重建与人工批准链（ACCEPTANCE）**。
- P2 未完成前，P3 及以后能力即使已有演示代码，也只能视为冻结的预研原型。
...
| **P2 可信空间重建与批准链** | **`ACCEPTANCE`** | 确定性语义提取、校正、验证和人工批准已形成验收候选 |
| P3-P10 | `BLOCKED` | 场景、硬装、资产、Viewer、Blender、修订、输入扩展和生产化 |
...
自动化证据位于 [examples/p2-acceptance/automated-evidence.json](examples/p2-acceptance/automated-evidence.json)。当前只缺需求门禁规定的人工验收结论；人工确认前 P2 仍不是 `COMPLETED`，P3-P10 仍不得开发。

本文对 P3-P10 的描述是目标设计，不构成阶段开放。当前已有 GLB 和 Viewer 代码只可用于验证 P2 输出兼容性或修复 P2 阻断问题。
